Shader "Universal Render Pipeline/2D/SpriteSheen2D"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float distWrap01(float v, float center)
            {
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                if (_Interval <= 0.0)
                {
                    float tCenter = frac(tSec * _Speed + _Offset);
                    float d = distWrap01(pRot.x, tCenter);
                    m = saturate((_SheenWidth - d) / max(_Softness, 1e-5));
                }
                else
                {
                    float inv = max(_Interval, 1e-3);
                    float phase = frac(tSec / inv + _Offset);
                    float activeRatio = saturate(_Duty);

                    float isActive = 1.0 - step(activeRatio, phase);
                    float local = saturate(phase / max(activeRatio, 1e-3));

                    float tCenter = lerp(-_SheenWidth, 1.0 + _SheenWidth, local);

                    float dNoWrap = abs(pRot.x - tCenter);

                    m = saturate((_SheenWidth - dNoWrap) / max(_Softness, 1e-5));
                    m *= isActive; 
                }
